      ...
      icon: (
        <svg width="14" height="14" viewBox="0 0 512 512" fill="none">
          <g clipPath="url(#oa-cl)">
            <path d="M439.173 216.769C448.876 187.692 445.534 155.84 430.018 129.392C406.682 88.826 359.771 67.956 313.956 77.777C293.575 54.852 264.292 41.815 233.593 42.002C186.762 41.895 145.211 72 130.803 116.488C100.719 122.64 74.751 141.442 59.555 168.09C36.046 208.549 41.405 259.55 72.813 294.244C63.11 323.321 66.451 355.173 81.968 381.621C105.303 422.187 152.214 443.057 198.029 433.236C218.397 456.161 247.693 469.198 278.392 468.998C325.25 469.118 366.815 438.987 381.222 394.458C411.307 388.306 437.275 369.505 452.471 342.857C475.953 302.397 470.58 251.437 439.186 216.742L439.173 216.769Z" fill="currentColor" />
            <path d="M278.419 441.095C259.668 441.122 241.505 434.57 227.111 422.574L229.637 421.146L314.798 372.04C319.155 369.571 321.828 364.941 321.802 359.937V240.067L357.793 260.817C358.181 261.004 358.435 261.378 358.488 261.805V361.071C358.435 405.213 322.63 441.002 278.419 441.095Z" fill="var(--background)" />
            <path d="M106.225 367.663C96.83 351.463 93.448 332.475 96.669 314.047L99.195 315.555L184.357 364.661C188.673 367.183 194.019 367.183 198.35 364.661L302.316 304.719V346.219C302.342 346.646 302.142 347.06 301.808 347.327L215.724 396.953C177.38 418.998 128.411 405.894 106.238 367.663Z" fill="var(--background)" />
            <path d="M83.812 182.061C93.168 165.835 107.936 153.425 125.524 146.98V149.915V248.141C125.457 253.131 128.13 257.762 132.474 260.23L236.44 320.158L200.448 340.908C200.087 341.149 199.633 341.189 199.232 341.015L113.135 291.349C74.871 269.224 61.747 220.345 83.799 182.074Z" fill="var(--background)" />
            <path d="M379.525 250.769L275.559 190.828L311.551 170.091C311.912 169.851 312.366 169.811 312.767 169.985L398.864 219.611C437.195 241.722 450.332 290.681 428.187 328.952C418.818 345.152 404.063 357.562 386.488 364.02V262.859C386.528 257.868 383.868 253.251 379.538 250.769Z" fill="var(--background)" />
            <path d="M415.343 196.94L412.817 195.432L327.656 146.326C323.339 143.804 317.993 143.804 313.662 146.326L209.696 206.267V164.767C209.67 164.34 209.87 163.926 210.204 163.66L296.288 114.073C334.632 91.989 383.655 105.133 405.76 143.43C415.102 159.603 418.484 178.538 415.316 196.94Z" fill="var(--background)" />
            <path d="M190.13 270.906L154.125 250.156C153.737 249.969 153.484 249.595 153.43 249.168V149.902C153.457 105.706 189.368 69.891 233.633 69.918C252.357 69.918 270.48 76.483 284.874 88.439L282.348 89.867L197.187 138.973C192.83 141.442 190.157 146.059 190.184 151.063L190.13 270.879Z" fill="var(--background)" />
            <path d="M209.683 228.818L255.993 202.117L302.302 228.805V282.195L255.993 308.883L209.683 282.195V228.818Z" fill="var(--background)" />
          </g>
          <defs>
            <clipPath id="oa-cl">
  ...
  [
    {
      ...
      icon: (
        <svg width="14" height="14" viewBox="0 0 120 120" fill="none">
          <path d="M60 10C30 10 15 35 15 55C15 75 30 95 45 100L45 110L55 110L55 100C55 100 60 102 65 100L65 110L75 110L75 100C90 95 105 75 105 55C105 35 90 10 60 10Z" fill="#E8553A" />
          <path d="M20 45C5 40 0 50 5 60C10 70 20 65 25 55C28 48 25 45 20 45Z" fill="#E8553A" />
          <path d="M100 45C115 40 120 50 115 60C110 70 100 65 95 55C92 48 95 45 100 45Z" fill="#E8553A" />
  ...
  return (
    ...
    >
      ...
      <span
        ...
        style={{ color: checked ? "oklch(0.35 0.06 145)" : "var(--foreground)" }}
      >
        {item.name}
      </span>
    </motion.span>
...
// The curve always fills 100% width but reshapes on each check.
// 0 checks = nothing. 1 check = nearly flat. All checks = hockey stick.
// Each check increases how much of the exponential we "see",
// so the right side shoots up more and more.
function GrowthCurve({ checkedCount }: { checkedCount: number }) {
  const id = useId();
  const W = 1000;
  const H = 270;
  ...
  // tMax controls how much of the exponential is visible.
  // Low tMax → flat line. High tMax → dramatic hockey stick.
  const tMax = 0.4 + 4.2 * (checkedCount / TOTAL);
  ...
  for (let i = 0; i <= steps; i++) {
    ...
    const x = frac * W;
    const t = frac * tMax;
    const norm = (Math.exp(t) - 1) / (Math.exp(tMax) - 1);
    // Subtle organic wiggle that decreases as curve steepens
    const wiggle = Math.sin(frac * 14) * 3 * (1 - norm);
    const y = H - H * 0.9 * norm + wiggle;
    const cmd = i === 0 ? "M" : "L";
    pts.push(`${cmd}${x.toFixed(1)},${y.toFixed(1)}`);
    areaPts.push(`L${x.toFixed(1)},${y.toFixed(1)}`);
  }

  const curveLine = pts.join(" ");
  const curveArea = `M0,${H} ${areaPts.join(" ")} L${W},${H} Z`;
    ...
    >
      <defs>
        ...
        <linearGradient id={`${id}-fy`} x1="0" y1="0" x2="0" y2="1">
          <stop offset="0%" stopColor="oklch(0.6 0.17 145)" stopOpacity="0.14" />
          <stop offset="100%" stopColor="oklch(0.6 0.17 145)" stopOpacity="0" />
        </linearGradient>
        {/* Horizontal mask — invisible left, visible right */}
        <linearGradient id={`${id}-mx`} x1="0" y1="0" x2="1" y2="0">
          <stop offset="0%" stopColor="white" stopOpacity="0" />
          <stop offset="30%" stopColor="white" stopOpacity="0.25" />
          <stop offset="65%" stopColor="white" stopOpacity="0.65" />
          <stop offset="100%" stopColor="white" stopOpacity="1" />
        </linearGradient>
        <mask id={`${id}-mask`}>
          <rect width={W} height={H} fill={`url(#${id}-mx)`} />
        </mask>
        {/* Stroke gradient — fades in left to right */}
        <linearGradient id={`${id}-sg`} x1="0" y1="0" x2="1" y2="0">
          <stop offset="0%" stopColor="oklch(0.6 0.17 145)" stopOpacity="0" />
          <stop offset="25%" stopColor="oklch(0.6 0.17 145)" stopOpacity="0.06" />
          <stop offset="55%" stopColor="oklch(0.6 0.17 145)" stopOpacity="0.18" />
          <stop offset="100%" stopColor="oklch(0.6 0.17 145)" stopOpacity="0.4" />
        </linearGradient>
      </defs>

      <g mask={`url(#${id}-mask)`}>
        <path d={curveArea} fill={`url(#${id}-fy)`} />
        <path d={curveLine} stroke={`url(#${id}-sg)`} strokeWidth="2" strokeLinecap="round" />
      </g>
    </motion.svg>
  );
}

// ── Label overlay (not stretched) ──────────────────────
function GraphLabel({ visible }: { visible: boolean }) {
  return (
    <motion.span
      className="absolute top-0.5 right-0 text-[10px] font-mono tracking-wider text-[oklch(0.5_0.15_145_/_0.4)] pointer-events-none"
      initial={{ opacity: 0 }}
...
export function ToolChecklist() {
  ...
  return (
    <div ref={containerRef} className="relative">
      {/* Background growth curve */}
      <GrowthCurve checkedCount={checkedCount} />
      <GraphLabel visible={checkedCount >= TOTAL} />
